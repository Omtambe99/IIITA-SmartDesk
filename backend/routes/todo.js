...
const {
  ...
  Student,
  Submission,
} = require("../models/index"); // Ensure this path matches your setup
  ...
  async (req, res) => {
    ...
    try {
      ...
      const updatedCourse = await Course.findByIdAndUpdate(
        ...
        { new: true }
      ); // Optionally populate if you need to return full assignment details
      ...
      res
        .status(500)
        .json({ message: "Failed to post assignment", error: err.message });
    }
  }
);
...
router.get("/:courseId", Authenticate, async (req, res) => {
  ...
  try {
    ...
    const course = await Course.findById(courseDetails._id).populate({
      path: "assignments",
      populate: [
        {
          path: "postedBy",
    ...
    res
      .status(500)
      .json({ message: "Failed to retrieve assignments", error: err.message });
  }
});
...
router.get("/courses/:courseId/assignments", async (req, res) => {
  ...
  try {
    ...
    if (!assignments.length) {
      return res
        .status(404)
        .send({ message: "No assignments found for this course." });
    }
