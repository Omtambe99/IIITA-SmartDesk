...
router.get("/:courseId", professorauthenticate, async (req, res) => {
  ...
  try {
    ...
    if (courseWithStudents.students.length === 0) {
      return res
        .status(404)
        .json({ msg: "No students enrolled in this course" });
    }
...
router.post("/:courseId", professorauthenticate, async (req, res) => {
  ...
  if (!Array.isArray(attendanceData)) {
    return res
      ...
      .json({
        message:
          "Invalid input format, expected an array of attendance records",
      });
  }
  ...
  try {
      ...
      return {
        updateOne: {
          filter: {
            ...
            "courses.course": courseDetails._id,
          },
          update: {
            $push: {
              "courses.$.attendance": {
    ...
    if (result.matchedCount === 0) {
      return res
        .status(404)
        .json({ message: "No students or courses found with given IDs" });
    }
